...
''';
  ...
  static Future<Map<String, dynamic>> askBuddy(String prompt) async {
    ...
    // Quick commands - Note
    if (p.startsWith('note:') || p.startsWith('create note') || p.startsWith('add note')) {
      final note = await EnhancedChatService.quickCreateNote(prompt);
      final confirm = FlowBuddyMessage(
        id: DateTime.now().millisecondsSinceEpoch.toString(),
        content: '📝 Note created: "${note.title}"',
        role: BuddyRole.assistant,
        timestamp: DateTime.now(),
        context: MessageContext.general,
      );
      _chatHistory.add(confirm);
      return {'response': confirm.content, 'note': note.toJson()};
    }

    // Quick commands - Task
    if (p.startsWith('task:') || p.startsWith('create task') || p.startsWith('add task')) {
      final task = await EnhancedChatService.quickCreateTask(prompt);
      final due = task.dueDate != null ? ' (due ${task.dueDate})' : '';
      final confirm = FlowBuddyMessage(
        id: DateTime.now().millisecondsSinceEpoch.toString(),
        content: '✅ Task created: "${task.title}"$due',
        role: BuddyRole.assistant,
        timestamp: DateTime.now(),
        context: MessageContext.general,
      );
      _chatHistory.add(confirm);
      return {'response': confirm.content, 'task': task.toJson()};
    }

    // Quick commands - Alarm/Reminder
    if (p.startsWith('alarm:') || p.startsWith('remind me') || p.startsWith('set a reminder') || p.startsWith('schedule')) {
      final alarm = await EnhancedChatService.quickCreateAlarm(prompt);
      final confirm = FlowBuddyMessage(
        id: DateTime.now().millisecondsSinceEpoch.toString(),
        content: '⏰ Reminder set: "${alarm.title}" at ${alarm.scheduledTime}',
        role: BuddyRole.assistant,
        timestamp: DateTime.now(),
        context: MessageContext.general,
      );
      _chatHistory.add(confirm);
      return {'response': confirm.content, 'alarm': alarm.toJson()};
    }

    // Flow creation
    if (isFlowCreationRequest(prompt)) {
      final description = extractProjectDescription(prompt);
      final createdFlow = await FlowService.generateFlowFromDescription(description);
      final msg = '✅ Flow Created Automatically!\n\n• Title: ${createdFlow.title}\n• Steps: ${createdFlow.checkpoints.length}\n• Duration: ${createdFlow.estimatedDuration}\n• Difficulty: ${createdFlow.difficulty.name.toUpperCase()}\n\n⏰ Alarms scheduled for each checkpoint\n📝 Notes added with requirements & deliverables\n\nOpen the Flows tab to start with "${createdFlow.checkpoints.isNotEmpty ? createdFlow.checkpoints.first.title : 'Step 1'}".';
      _chatHistory.add(
        FlowBuddyMessage(
          id: DateTime.now().millisecondsSinceEpoch.toString(),
          content: msg,
    ...
    // Confirmation and checkpoint help
    ...
    try {
      ...
      final resp = await http.post(
        ...
        body: jsonEncode({
          'prompt': prompt,
          'chat_history': _chatHistory.map((m) => m.toJson()).toList(),
        }),
      );

      if (resp.statusCode == 200) {
        final data = jsonDecode(resp.body);
        final ai = data['response'] ?? 'No response';
    ...
    // Fallback response
    final fb = _generateFallbackResponse(prompt);
    final assistantMessage = FlowBuddyMessage(
      id: DateTime.now().millisecondsSinceEpoch.toString(),
      content: fb,
      role: BuddyRole.assistant,
  ...
  static Future<Map<String, dynamic>> _handleFlowConfirmation(
    ...
    final flowMessages = _chatHistory.where((msg) => msg.flowData != null).toList();
    final lastFlowMessage = flowMessages.isNotEmpty ? flowMessages.last : null;
